...
namespace MyBlog.Engine
{
    public sealed class DataService:IDisposable
    {
        ...
        public static void Initilize(DataContext context)
        {
            ...
            // Database.SetInitializer(new MigrateDatabaseToLatestVersion<DataContext, MyBlog.Engine.Migrations.Configuration>());
        ...
        public PostWithoutContent[] GetPostsInArchive(ArchiveId id, Int32 offset)
        {
            ...
            return _context.Posts
                .Include(CategoriesPropertyName).Include(CategoriesAndCategoryPropertyName)
                .Where(predicate)
                .OrderByDescending(c => c.DateCreatedGmt)
                .Skip(offset)
            ...
            return _context.PostCategories
                ...
                .Select(c =>
                    new PostWithoutContent
                    {
                        Id = c.Post.Id,
                        Title = c.Post.Title,
                        Categories = c.Post.Categories.Select(cat => cat.Category).ToArray(),
                        DateCreatedGmt = c.Post.DateCreatedGmt,
                        Url = GetPostUrl(c.Post.Id, c.Post.Title)
                    }
                ).ToArray();
        }
